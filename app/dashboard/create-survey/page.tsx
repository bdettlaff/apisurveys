...
export default function CreateSurveyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch("http://localhost:8080/api/admin/surveys", {
      method: "POST",
      headers: {
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <main className="flex flex-col items-center pt-32 px-6">
        <div className="w-full max-w-3xl p-10 bg-white rounded-2xl shadow-xl border border-zinc-200">
          <h1 className="text-3xl font-black mb-8">Tworzenie ankiety</h1>
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="font-semibold">Data rozpoczęcia</label>
              <input
            ...
            <div>
              <label className="font-semibold">Data zakończenia</label>
              <input
                className="border p-3 rounded-xl w-full mt-1"
                type="date"
                value={endDate}
            ...
            <div className="border rounded-2xl p-5 mt-4 bg-zinc-50">
              <h2 className="text-xl font-bold mb-3">Podgląd ankiety</h2>
              <p>Klasa: {selectedClass?.name || "Nie wybrano"}</p>
              <p>Data rozpoczęcia: {startDate || "Nie wybrano"}</p>
              <p>Data zakończenia: {endDate || "Nie wybrano"}</p>
              <h3 className="font-bold mt-5 mb-2">Bloki oceny:</h3>
              {blocks.length === 0 ? (
                <p className="text-zinc-500">Brak bloków do wyświetlenia.</p>
              ) : (
                <ul className="flex flex-col gap-2">
                  {blocks.map((block) => (
                    ...
                    >
                      <p className="font-semibold">{block.teacherName}</p>
                      <p className="text-sm text-zinc-600">
                        {block.subjectName}
                      </p>
                      <p className="text-xs text-zinc-400">
                        Moduł pytań: {block.module}
                      </p>
                    </li>
                  ))}
                </ul>
