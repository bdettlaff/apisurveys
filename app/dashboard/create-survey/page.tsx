...
type Question = { id: string; content: string; type: string; category: Category };
type SurveyBlock = {
  id: number;
...
export default function CreateSurveyPage() {
  ...
  const [selectedQuestionsPerBlock, setSelectedQuestionsPerBlock] = useState<Record<string, string[]>>({});
    ...
    if (!classId) {
      ...
      fetch(`http://localhost:8080/api/classes/${classId}/survey-blocks`)
        ...
        .then((data: SurveyBlock[]) => {
          ...
          data.forEach((block) => {
            const blockKey = block.isSchoolSection ? "school-general-section" : String(block.id);
            initialSelection[blockKey] = (block.questions || []).map((q) => q.id);
          });
          setSelectedQuestionsPerBlock(initialSelection);
        })
        .catch(() => { setBlocks([]); setSelectedQuestionsPerBlock({}); });
    }
  }, [classId]);
  ...
  const handleToggleQuestion = (blockKey: string, questionId: string) => {
    ...
    if (currentSelected.includes(questionId)) {
      setSelectedQuestionsPerBlock({ ...selectedQuestionsPerBlock, [blockKey]: currentSelected.filter((id) => id !== questionId) });
    } else {
      setSelectedQuestionsPerBlock({ ...selectedQuestionsPerBlock, [blockKey]: [...currentSelected, questionId] });
    }
  };

  const totalSelectedQuestionsCount = useMemo(() => {
    return Object.values(selectedQuestionsPerBlock).reduce((acc, curr) => acc + curr.length, 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const teacherSurveys = blocks
      ...
      .filter((survey) => survey.questionIds.length > 0 && survey.teacherId && survey.subjectId);

    const schoolQuestionIds = selectedQuestionsPerBlock["school-general-section"] || [];
    setIsSubmitting(true);

    try {
      const response = await fetch("http://localhost:8080/api/admin/surveys/composite", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ classId: Number(classId), startDate, endDate, teacherSurveys, schoolQuestionIds }),
      });
      ...
      <div className="min-h-screen bg-zinc-50">
        ...
        <main className="max-w-xl mx-auto pt-32 px-4 text-center">
          <div className="bg-white rounded-2xl border border-red-100 p-8 shadow-sm">
            <h1 className="text-sm font-black text-red-600 uppercase tracking-wider">Brak Dostępnych Uprawnień</h1>
          </div>
        </main>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-zinc-50 antialiased text-zinc-900 selection:bg-zinc-200">
      ...
      <main className="max-w-7xl mx-auto px-4 pt-28 pb-16">
        <div className="mb-8">
          <h1 className="text-3xl font-black tracking-tight uppercase text-zinc-900">Zarządzanie Ankietami</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
          <div className="lg:col-span-2 bg-white border border-zinc-200/60 rounded-2xl p-6 shadow-sm">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-zinc-700 mb-2">Wybierz oddział</label>
                <select
                  value={classId}
                  onChange={(e) => { setClassId(e.target.value); setGeneratedCode(null); }}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-zinc-700 mb-2">Data startu</label>
                  <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                    className="w-full px-4 py-3 border border-zinc-200 rounded-xl text-sm font-medium text-zinc-700" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-zinc-700 mb-2">Data końca</label>
              ...
              <div className="pt-4 border-t border-zinc-100">
                <button type="submit" disabled={isSubmitting || blocks.length === 0}
                  ...
                  {isSubmitting ? "Zapisywanie struktury..." : `Zapisz ${blocks.length} sekcji w systemie`}
                </button>
              </div>
            </form>
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-zinc-900 text-white rounded-2xl p-6 shadow-xl">
              <h2 className="text-xs font-black tracking-widest text-zinc-400 uppercase mb-4">Stan struktury pytań</h2>
              <div className="space-y-3 text-xs">
                <div className="flex justify-between border-b border-zinc-800 pb-2">
                  <span className="text-zinc-400">Aktywny oddział:</span>
                  <span className="font-bold text-indigo-400">{selectedClass?.name || "Nie wybrano"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-400">Suma wybranych pytań:</span>
                  <span className="font-bold text-emerald-400">{totalSelectedQuestionsCount}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
              {blocks.length === 0 ? (
                <div className="bg-white text-center py-16 border border-zinc-200/60 rounded-2xl text-zinc-400 text-sm">
                  Wskaż oddział szkolny, aby pobrać dynamiczne zestawy pytań z bazy danych.
                </div>
              ) : (
                blocks.map((block) => {
                  const blockKey = block.isSchoolSection ? "school-general-section" : String(block.id);
                  ...
                  return (
                    <div key={blockKey} className={`border rounded-2xl p-5 shadow-sm space-y-3 ${block.isSchoolSection ? "bg-indigo-50/20 border-indigo-100" : "bg-white border-zinc-200/60"}`}>
                      <div className="flex justify-between items-start border-b border-zinc-100 pb-3">
                        <div>
                          <h3 className={`text-sm font-black ${block.isSchoolSection ? "text-indigo-950" : "text-zinc-900"}`}>{block.teacherName}</h3>
                          <p className="text-xs font-semibold text-zinc-400 uppercase mt-0.5">
                            {block.subjectName} • <span className="font-bold text-zinc-600 bg-zinc-100 px-1.5 py-0.5 rounded text-[10px]">{block.module}</span>
                          </p>
                        </div>
                        <span className="bg-zinc-100 border text-[10px] font-bold px-2 py-1 rounded-md text-zinc-700">{selectedIds.length} aktywnych</span>
                      </div>
                      <div className="space-y-1.5">
                        {!block.questions || block.questions.length === 0 ? (
                          <p className="text-[11px] text-zinc-400 italic py-1">Brak przypisanych pytań w tej kategorii przedmiotowej.</p>
                        ) : (
                          block.questions.map((q) => {
                            const isChecked = selectedIds.includes(q.id);
                            return (
                              <label key={q.id} className={`flex items-start gap-3 p-2.5 rounded-xl border text-xs cursor-pointer transition-all select-none ${isChecked ? block.isSchoolSection ? "bg-indigo-50/40 border-indigo-200 text-zinc-800" : "bg-emerald-50/30 border-emerald-100 text-zinc-800" : "bg-zinc-100/40 border-zinc-200 text-zinc-400/70 opacity-60"}`}>
                                <input type="checkbox" checked={isChecked} onChange={() => handleToggleQuestion(blockKey, q.id)}
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="font-medium break-words leading-tight">
                                    <span className="font-mono font-bold mr-1 text-[10px] border px-1 rounded bg-zinc-100 text-zinc-900">{q.id}</span>
                                    {q.content}
                                  </p>
                                </div>
                              </label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 border border-zinc-200">
            ...
            <p className="text-center text-sm text-zinc-500 mb-6">
              Kod dostępu dla klasy <span className="font-bold text-zinc-800">{savedClassName}</span>:
            </p>
            ...
            <p className="text-xs text-center text-zinc-400 mb-5">
              Przekaż ten kod uczniom — pozwoli im zobaczyć przypisane ankiety.
            </p>
